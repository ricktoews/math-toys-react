...
var total = 0;
var tripleCounts = {};
for (let c = 5; c <= 1000000; c++) {
	let layers = checkWrap(c);
	let squares = findSquares(layers);
	let organized = organizeSquares(c, squares);
	if (organized.length > 0) {
		if (!tripleCounts[organized.length]) tripleCounts[organized.length] = 0;
		tripleCounts[organized.length]++;
//		console.log(`c = ${c}, c^2 = ${c*c}`, organized.length);
		tally++;
	}
	total++;
	if (total % 10000 === 0) {
console.log(JSON.stringify(tripleCounts));
	}
}
console.log(tripleCounts);
